...
def tokenize_reduce_hybrid(reduce_command, detect_exception=True):
    assert reduce_command[0]=='R'
    tokens = reduce_command.split('-')
    if detect_exception and "=" in tokens[-1]:
        print '----------detecting = -------------'
        print tokens[-1]
        temp = tokens[-1].split('=')
        tokens[-1] = temp[0]
    elif detect_exception and "=" in ' '.join(tokens[1:]):
        print '=========EXCEPTION=============='
        print tokens[1:]
...
def main(f_input, f_output):
    # '''
    input_sentence = 'S S S R3-NP R2-PP-TMP=5 S S S R5-S'
